...
- সফল হলে gateway লোকালি `http://localhost:8642` এ চলবে — `curl http://localhost:8642/health` দিয়ে চেক করো
...
- `resources/hermes-config.json`-এ gateway URL ঠিক আছে কিনা দেখো (dev-এ লোকাল
  gateway টেস্ট করতে চাইলে `http://localhost:8642` / `ws://localhost:8642`)
