...
Point any OpenAI-compatible client at `https://responsible-purpose-production-e6fd.up.railway.app/v1`:

```bash
# Test with curl
curl https://responsible-purpose-production-e6fd.up.railway.app/v1/chat/completions \
...
curl -X POST https://responsible-purpose-production-e6fd.up.railway.app/api/sessions/$ID/fork \
  -H "Authorization: Bearer $API_SERVER_KEY" \
  -d '{"title": "explore alt path"}'

# stream a turn over SSE
curl -N -X POST https://responsible-purpose-production-e6fd.up.railway.app/api/sessions/$ID/chat/stream \
  -H "Authorization: Bearer $API_SERVER_KEY" \
...
```bash
curl https://responsible-purpose-production-e6fd.up.railway.app/v1/skills \
  -H "Authorization: Bearer $API_SERVER_KEY"
# → [{"name": "github-pr-workflow", "description": "...", "category": "..."}, ...]

curl https://responsible-purpose-production-e6fd.up.railway.app/v1/toolsets \
  -H "Authorization: Bearer $API_SERVER_KEY"
...
| OpenAI Python SDK | — | `OpenAI(base_url="https://responsible-purpose-production-e6fd.up.railway.app/v1")` |
