...
- **Verify the gateway is running**: `curl https://responsible-purpose-production-e6fd.up.railway.app/health` should return `{"status": "ok"}`
- **Check model listing**: `curl -H "Authorization: Bearer your-secret-key" https://responsible-purpose-production-e6fd.up.railway.app/v1/models` should return a list with `hermes-agent`
...
docker run --network=host -e OPENAI_API_BASE_URL=https://responsible-purpose-production-e6fd.up.railway.app/v1 ...
