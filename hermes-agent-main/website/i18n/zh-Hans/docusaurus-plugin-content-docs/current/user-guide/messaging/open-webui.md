...
- **验证 gateway 是否运行**：`curl https://responsible-purpose-production-e6fd.up.railway.app/health` 应返回 `{"status": "ok"}`
- **检查模型列表**：`curl -H "Authorization: Bearer your-secret-key" https://responsible-purpose-production-e6fd.up.railway.app/v1/models` 应返回包含 `hermes-agent` 的列表
