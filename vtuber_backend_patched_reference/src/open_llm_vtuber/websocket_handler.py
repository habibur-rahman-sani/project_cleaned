...
class WebSocketHandler:
    ...
    async def handle_new_connection(
        ...
        Raises:
            Exception: If initialization fails, OR if multi-user routing
            fails/is missing — this server has no single-user fallback,
            so a connection without a valid token is always rejected.
        """
        try:
            ...
            # --- মাল্টি-ইউজার: বাধ্যতামূলক। এই প্রজেক্টে single-user/token-less
            #     কানেকশন সাপোর্টেড না — token না থাকলে বা gateway routing
            #     ব্যর্থ হলে কানেকশনটাই বাতিল, conf.yaml-এর ডিফল্ট LLM কখনো
            #     ব্যবহার হবে না। ---
            if not token:
                raise RuntimeError(
                    "কোনো token ছাড়া কানেকশন প্রত্যাখ্যান করা হলো — এই সার্ভার "
                    "শুধুই মাল্টি-ইউজার মোডে চলে (ws://.../client-ws?token=<gateway token>)।"
                )
            ...
            try:
                ...
                pass  # ততক্ষণে সকেট বন্ধ হয়ে গেলে এটা ব্যর্থ হতেই পারে, গুরুত্বপূর্ণ না
